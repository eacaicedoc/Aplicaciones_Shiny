...
shinyServer(function(input, output, session) {
  ...
  Tamano <- function(base1,base2) {
    ...
    if ((n<=30) | (m<=30)) {
      ...
      T0 <- N*(t(Mm1-Mm2))%*%Scom%*%(Mm1-Mm2)
      
      p <- length(Mm1) 
      a <- ((n+m-2)*p)/(n+m-p-1)
      
      #Valor p de la prueba
      value <- pf(q=T0/a, df1=p, df2=n+m-p-1, lower.tail=F)
      
      res <- list(estadistico=T0, n=n, df1=p, df2=n+m-p-1, m=m, Mm1=Mm1,
                  Mm2=Mm2, S=Sp, value=value, Scom=Scom, N=N, a=a)
    }
      ...
      res <- list(estadistico=X0, n=n, df=p, m=m, Mm1=Mm1, 
