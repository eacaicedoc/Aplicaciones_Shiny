...
shinyUI(pageWithSidebar(
  ...
  sidebarPanel(
    h5('Esta aplicación permite realizar una prueba de hipótesis sobre 
       el vector de medias poblacionales de una distribución normal multivariada.'),
    
    h6('La aplicación por defecto usa los datos del ejemplo 5.2 página 214 
        de Jhonson & Wichern (2007) Applied Multivariate Statistical Analysis, 
       sin embargo, el usuario puede subir su propia base de datos para usar la app.'),
    ...
    checkboxGroupInput(inputId="names", 
                       label=" Elija la variables cuantitativa para realizar la prueba 
                       de hipótesis.", 
                       choices=c("x1","x2","x3")),
   ...
   p(strong(HTML("Ingrese el valor de referencia &mu;<sub>0</sub> para probar
    ...
    tags$textarea(id="vectorIng", cols=20, rows=2, "4 50 10"),
   br(),
